The time evolution of systems towards equilibrium in governed by the ==second law==. For instance, for an **[[Adiabatically Isolated System|adiabatically isolated system]]**, ==entropy== must increase in any spontaneouschange and reaches a maximun in equilibrium. 

For a system with no heat exchange that approaches mechanical equilibrium with constant externally applied ==generalized forces==, the conditions are $\mathrm{\bar{d}}Q$=0 and $\delta \mathbf{J}=0$. For any set of ==generalized displacements== $\mathbf{x}=\left( x_{1},x_{2},\dots \right)$, at constant externally applied generalized forces $\mathbf{J}=\left( J_{1},J_{2},\dots \right)$, while going through a reversible or irreversible transformation, the work input to the system $\mathrm{\bar{d}}W$ satisfies

$$
\mathrm{\bar{d}}W\leq \mathbf{J} \cdot \delta\mathbf{x}.
\tag 1
$$ 
Equality holds for a ==reversible transformation==, for which the applied generalized forces match the internal generalized forces. However, for ==irreversible transformations==, there is usually some loss of the external work to dissipation. Thus, the actual work contributing to the ==internal energy== change of the system $\mathrm{\bar{d}}W$ is generally smaller than $\mathbf{J}\cdot \delta \mathbf{x}$.

Since $\mathrm{\bar{d}}Q=0$, according to the ==first law==, we obtain
...
The variations of $H$ during a reversible tranformation are given by
...
In particular, for an ==ideal gas==, the Equations $(5)$, $(6)$, and $(7)$ reudce to
...
Note that heat capacity with constant pressure can be expresse as 
...
For a system going through ==isothermal transformations== in the absence of the machanical work, the conditions are $\mathrm{\bar{d}}W=0$ and $\delta T=0$. From ==Clausius theorem==, the heat intake of the system at a constant temperature $T$ satisfies $\mathrm{\bar{d}}Q\leq T\delta S$. Thus, we obtain
...
The variations of $F$ during a reversible tranformation are given by
...
The internal energy can also be calculate from $F$ using
...
Variations of $G$ during a reversible tranformation are given by
...
In thermodynamics, the appropriate state function is chosen according to the constrains imposed by the surroundings. For a simple compressible system, which satisfies $\mathbf{J}=-P$ and $\mathbf{x}=V$, with constant bolume and the numbers of particles, the internal energy $E(S,V,\mathbf{N})$ is the natural potential when $S$, $V$, and $\mathbf{N}$ are specified. If the system is thermal contact with a hear reservoir, the temperature $T$ rather than the entropy $S$ is controlled, and the Helmholtz free energy $F(T,V,\mathbf{N})$ becomes the appropriate potential. If the external pressure $-P$ is fixed instead of the volume $V$, the enthalpy $H(S,P,\mathbf{N})$ is natural. If both temperature $T$ and pressure $-P$ are fixed, the Gibbs free energy $G(T,P,\mathbf{N})$ is the appropriate potential. When the system is also in contact with a particle reservoir, the partical numbers $\mathbf{N}$ are replaced by their conjugate chemical potential $\boldsymbol{\mu}$. 
