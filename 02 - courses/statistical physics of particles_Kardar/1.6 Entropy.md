**[[Clausius's Theorem]]**. For any **[[Cyclic Transformation|cyclic transformation]]**, whether reversible or not, the relation
...
Thus, for [[Reversible Process|reversible transformations]], we find that the integral $\int \mathrm{\bar{d}}Q / T$ between two equilibrium states is independent of the path. Therefore, it defines a new state function $S$, known as **[[Entropy|entropy]]**, which satisfies
...
Using entropy, we can now calculate the heat exchanged during a reversible transformation from $\mathrm{\bar{d}}Q=T\mathrm{d}S$ and construct adiabatic curves for a general system since $\mathrm{\bar{d}}Q=0$ leads to $\mathrm{d}S=0$. As reversible processes are quasi-static,  for reversible processes, the equation $\mathrm{\bar{d}}W=\sum_{i}J_{i}\mathrm{d}x_{i}$ holds. According to the [[1.3 the First Law|first law]], we can obtain the internal energy from
...
In other words, for an irreversible transformation from $A$ to $B$, the inequality
