...
The first law rules out "perpetual motion machine of the first kind", which can produce work without consuming any energy. However, the conservation of energy is not violated by an engine, for example, which produces work by using the energy released when water freezes into ice. This kind of engine is called "perpetual motion machine of the second kind", and its feasibility would still be constrained by the second law. 
